(function(){
	...
	haiyunControllers.controller('allDocListCtrl', ['$scope','httpService','alertService', function($scope,httpService,alertService) {
		...
		var init = function(){
			//$scope.statusList = [{'name':'未开始','value':'NOSTART'},{'name':'进行中','value':'ON'},{'name':'已过期','value':'OUTOFDATE'},{'name':'已废弃','value':'DROP'}];
			$scope.dataList.stateList = [
				{'name':{'cnName':'待投标','enName':'WAITING_BIDS'}[name],'value':'WAITING_BIDS'},
		...
		var getDocList = function(){
			...
			var input = {
				...
				'limit':size,
				//'biddoc.state':'WAITING_BIDS',//'WAITING_OR_NEGOTIATION'
				'biddoc.docType':'PERSONAL'
			};
			//!!!$scope.state||$scope.state==='ALL'?{}:input['biddoc.state'] = $scope.state;
			if(!!$scope.input.arrivalPortID){input['biddoc.arrivalPortID']=$scope.input.arrivalPortID;}
			if(!!$scope.input.departurePortID){input['biddoc.departurePortID']=$scope.input.departurePortID;}
			if(!!$scope.input.status){input['biddoc.state']=$scope.input.status;}
