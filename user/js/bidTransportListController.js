(function(){
	...
	haiyunControllers.controller('bidTransportListController', ['$scope','httpService','alertService', function($scope,httpService,alertService) {
		$scope.page = 1;
		$scope.pageSize = 1;
		var size = 10;
		...
		$scope.changeState = function(id,state){
			httpService.doPost('./struts/updateBidOfferState.action', {'bidOffer.id':id,'bidOffer.state':state.value}, function(ret){
				alertService.myAlert({'cnName':"修改成功！",'enName':'change successfully!'}[name],'success');
			});
		};
